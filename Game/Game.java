...
public class Game extends JFrame {
	...
	class keyProcess implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			else if (KeyLeft == true && bar.getX() > 10)
				bar.moveL();
		}
	...
	private class Bar{
		...
		public Bar() {
			bX = PANEL_WIDTH/2-BAR_WIDTH/2;
			bY = PANEL_HEIGHT-BAR_HEIGHT-65;
			width = BAR_WIDTH;
			height = BAR_HEIGHT;
		}
		public void moveR() {
		...
		// 어떤 주어진 좌표 p 와 그림의 중점과의 거리 (그림의 좌표는 왼쪽 모서리이므로... 폯과 높이를 감안하여 중점을 사용)
		public double distance (Point p) {
			return Math.sqrt(Math.pow((bX+width/2)-p.x, 2)+ Math.pow((bY+height/2)-p.y, 2));
		}
	}
